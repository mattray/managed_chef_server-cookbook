...
describe command('knife data bag list -c /etc/opscode/managed/chef_managed_org/config.rb') do
  its ('stdout') { should match /^users$/ }
  its ('stdout') { should match /^tests$/ }
end

describe command('knife data bag show users -c /etc/opscode/managed/chef_managed_org/config.rb') do
...
describe command('knife data bag show users user2 -c /etc/opscode/managed/chef_managed_org/config.rb') do
  its ('stdout') { should match /^name: User 2$/ }
end

describe command('knife data bag show users user3 -c /etc/opscode/managed/chef_managed_org/config.rb') do
  its ('stdout') { should match /^name: User Three$/ }
end

describe command('knife data bag show tests -c /etc/opscode/managed/chef_managed_org/config.rb') do
